...
class Kohana_Casper {
    ...
    private $currentHtml = '';
    private $loadTime = '';
    private $tempDir = DOCROOT;
    private $path2casper = NULL; //path to CasperJS
    private $headers = [];
    private $status;
    private $statusText = '';
    private $cookies = [];
...
";
    ...
    public static function setViewPort($width, $height)
    {
...
FRAGMENT;
     ...
     * @throws \Exception
     */
    public function run($isInfinite = NULL)
    {
...
FRAGMENT;
        ...
        if ($isInfinite == TRUE) {
                ...
                exec($this->path2casper . "casperjs " . $filename . "  > /dev/null &");
                ...
                exec($this->path2casper . "casperjs " . $filename . " &", $output);
...
casper.then(function(){
    fs.write('$file', this.getPageContent(), 'w');
});
...
FRAGMENT;
    ...
    public static function factory()
    {
        ...
        $path2casper = $config['path2casper'];
        $tempDir = $config['tempDir'];

        return new Casper($path2casper, $tempDir);
    }
}
